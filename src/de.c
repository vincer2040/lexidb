...
static uint8_t api_add_event(De* de, int fd, uint32_t flags) {
    ...
    int opt;
    api = de->api;
    opt = de->events[fd].flags == DE_NONE ? EPOLL_CTL_ADD : EPOLL_CTL_MOD;
    ee.events = 0;
    ...
    if (epoll_ctl(api->epollfd, opt, fd, &ee) == -1) {
        fmt_error("could not add event (errno: %d)\n%s\n", errno, strerror(errno));
        return 1;
    }
    return 0;
}
...
static int fire_events(De* de) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (invert) {
            if ((dfe->flags & flags & DE_READ) &&
                (!fired || (dfe->write_fn != dfe->read_fn))) {
                dfe->read_fn(de, fd, dfe->client_data, flags);
                fired++;
            }
        }
...
uint8_t de_add_event(De* de, int fd, uint32_t flags, DeFileFn* fn,
                     void* client_data) {
    DeFileEvent* dfe;
    if (fd >= de->nnem) {
        return 1;
